...
ASSET_DIR = DOCS_DIR / "discussion_guide_assets"
...
def _safe_get(data: dict[str, Any], path: str, default: Any = "Not available") -> Any:
    current: Any = data
...
def _existing_image_marker(path: Path, caption: str) -> str:
    if path.exists():
        relative = path.relative_to(PROJECT_ROOT).as_posix()
        return f"\n[[IMAGE:{relative}|{caption}]]\n"
    return f"\n> Figure missing: {caption} ({path.name})\n"


def _discover_files() -> dict[str, list[str]]:
    groups = {
        "root": ["main.py", "config.py", "gui_app.py", "requirements.txt"],
        "numerical": [],
        "image": [],
        "gui": [],
        "outputs": [],
    }
    for path in sorted((PROJECT_ROOT / "src" / "numerical").glob("*.py")):
        groups["numerical"].append(path.relative_to(PROJECT_ROOT).as_posix())
    for path in sorted((PROJECT_ROOT / "src" / "image").glob("*.py")):
        groups["image"].append(path.relative_to(PROJECT_ROOT).as_posix())
    for path in sorted((PROJECT_ROOT / "gui").glob("*.py")):
        groups["gui"].append(path.relative_to(PROJECT_ROOT).as_posix())
    for path in sorted((PROJECT_ROOT / "outputs").rglob("*")):
        if path.is_file() and path.suffix.lower() in {".json", ".txt", ".png", ".joblib"}:
            groups["outputs"].append(path.relative_to(PROJECT_ROOT).as_posix())
    return groups
...
def _markdown(results: dict[str, Any]) -> str:
    ...
    numerical_meta = _safe_get(results, "numerical.metadata", {})
    image_meta = _safe_get(results, "image.metadata", {})
    linear = _safe_get(results, "numerical.linear_regression", {})
    knn = _safe_get(results, "numerical.knn_regressor", {})
    logistic = _safe_get(results, "image.logistic_regression", {})
    kmeans = _safe_get(results, "image.kmeans", {})

    num_features = numerical_meta.get("raw_feature_columns", [])
    num_numeric = numerical_meta.get("numeric_features", [])
    num_categorical = numerical_meta.get("categorical_features", [])
    image_classes = image_meta.get("class_labels", [])
    image_counts = image_meta.get("class_counts", {})
    encoded_feature_count = numerical_meta.get("number_of_extracted_features", "Not available")
    image_feature_count = image_meta.get("number_of_extracted_features", "Not available")

    linear_test = linear.get("test_metrics", {})
    knn_test = knn.get("test_metrics", {})
    logistic_test = logistic.get("test_metrics", {})
    kmeans_test = kmeans.get("test_metrics", {})

    figure_section = "\n".join(
        [
            _existing_image_marker(
                PROJECT_ROOT / "outputs" / "figures" / "numerical_target_distribution.png",
                "Numerical target distribution.",
            ),
            _existing_image_marker(
                PROJECT_ROOT / "outputs" / "figures" / "linear_regression_predicted_vs_actual.png",
                "Linear Regression predicted values compared with actual values.",
            ),
            _existing_image_marker(
                PROJECT_ROOT / "outputs" / "figures" / "knn_regressor_predicted_vs_actual.png",
                "KNN Regressor predicted values compared with actual values.",
            ),
            _existing_image_marker(
                PROJECT_ROOT / "outputs" / "figures" / "linear_regression_residuals.png",
                "Linear Regression residual plot.",
            ),
            _existing_image_marker(
                PROJECT_ROOT / "outputs" / "figures" / "knn_regressor_residuals.png",
                "KNN Regressor residual plot.",
            ),
            _existing_image_marker(
                PROJECT_ROOT / "outputs" / "figures" / "regression_model_comparison.png",
                "Regression model comparison chart.",
            ),
            _existing_image_marker(
                PROJECT_ROOT / "outputs" / "figures" / "image_model_comparison.png",
                "Image model accuracy comparison chart.",
            ),
            _existing_image_marker(
                PROJECT_ROOT / "outputs" / "confusion_matrices" / "logistic_regression_confusion_matrix.png",
                "Logistic Regression confusion matrix.",
            ),
            _existing_image_marker(
                PROJECT_ROOT / "outputs" / "confusion_matrices" / "kmeans_confusion_matrix.png",
                "KMeans mapped-cluster confusion matrix.",
            ),
        ]
    )

    output_files = "\n".join(f"- `{name}`" for name in files["outputs"][:60])
    if len(files["outputs"]) > 60:
        output_files += f"\n- ... and {len(files['outputs']) - 60} more output files."

    return f"""# Machine Learning Project Discussion Preparation Guide
...
For the numerical part, the target variable is `{numerical_meta.get("target_column", "Not available")}` from the dataset `{numerical_meta.get("dataset_name", "Not available")}`. The goal is to predict a continuous numeric value, so the correct evaluation metrics are MAE, MSE, RMSE, and R2.

For the image part, the dataset is `{image_meta.get("dataset_name", "Not available")}`. The final selected classes are: {", ".join(image_classes) if image_classes else "Not available"}. The goal is to classify an input image into one of these class labels. Since this is classification, the correct evaluation metrics are accuracy, confusion matrix, precision, recall, and F1-score.
...
{chr(10).join(f"- `{name}`" for name in files["root"])}

Numerical source files:

{chr(10).join(f"- `{name}`" for name in files["numerical"])}

Image source files:

{chr(10).join(f"- `{name}`" for name in files["image"])}

GUI files:

{chr(10).join(f"- `{name}`" for name in files["gui"])}
...
The numerical dataset used in the current run is `{numerical_meta.get("dataset_name", "Not available")}`. The file is loaded from `datasets/numerical/salaries_cyber.csv`.

Dataset details from the saved results:

- Original shape: {_fmt(numerical_meta.get("original_shape", "Not available"))}
- Used shape after dropping configured non-feature columns: {_fmt(numerical_meta.get("used_shape", "Not available"))}
- Total samples: {_fmt(numerical_meta.get("split_counts", {}).get("total"))}
- Training samples: {_fmt(numerical_meta.get("split_counts", {}).get("train"))}
- Validation samples: {_fmt(numerical_meta.get("split_counts", {}).get("validation"))}
- Test samples: {_fmt(numerical_meta.get("split_counts", {}).get("test"))}
- Target variable: `{numerical_meta.get("target_column", "Not available")}`
- Raw input features: {", ".join(num_features) if num_features else "Not available"}
- Numerical features: {", ".join(num_numeric) if num_numeric else "Not available"}
- Categorical features: {", ".join(num_categorical) if num_categorical else "Not available"}
- Final encoded feature count saved in metadata: {_fmt(encoded_feature_count)}

The target distribution is important because salary-like data often contains extreme values. In this run, the target skewness was {_fmt(numerical_meta.get("target_skewness"))}. The code checked whether `log1p` transformation should be used. It was considered because the target was skewed, but the final saved results show `selected_log1p_transform = {linear.get("hyperparameters", {}).get("selected_log1p_transform", "Not available")}` for Linear Regression and `{knn.get("hyperparameters", {}).get("selected_log1p_transform", "Not available")}` for KNN. The pipeline chose the transformation only if validation RMSE improved.
...
{_existing_image_marker(PROJECT_ROOT / "outputs" / "figures" / "numerical_target_distribution.png", "Target distribution used to inspect skewness and outliers.")}
...
- `fit_intercept`: {linear.get("hyperparameters", {}).get("fit_intercept", "Not available")}
- `positive`: {linear.get("hyperparameters", {}).get("positive", "Not available")}
- `selected_log1p_transform`: {linear.get("hyperparameters", {}).get("selected_log1p_transform", "Not available")}
...
- Cross-validation: {knn.get("hyperparameters", {}).get("cv_folds", "Not available")}-fold CV.
...
- `n_neighbors`: {knn.get("hyperparameters", {}).get("selected_n_neighbors", "Not available")}
- `weights`: {knn.get("hyperparameters", {}).get("selected_weights", "Not available")}
- `metric`: {knn.get("hyperparameters", {}).get("selected_metric", "Not available")}
- `selected_log1p_transform`: {knn.get("hyperparameters", {}).get("selected_log1p_transform", "Not available")}
...
|---|---:|---:|---:|---:|
| Linear Regression | {_fmt(linear_test.get("MAE"))} | {_fmt(linear_test.get("MSE"))} | {_fmt(linear_test.get("RMSE"))} | {_fmt(linear_test.get("R2"))} |
| KNN Regressor | {_fmt(knn_test.get("MAE"))} | {_fmt(knn_test.get("MSE"))} | {_fmt(knn_test.get("RMSE"))} | {_fmt(knn_test.get("R2"))} |
...
The Linear Regression test RMSE is {_fmt(linear_test.get("RMSE"))}, while the KNN test RMSE is {_fmt(knn_test.get("RMSE"))}. The Linear Regression R2 is {_fmt(linear_test.get("R2"))}, while the KNN R2 is {_fmt(knn_test.get("R2"))}.
...
{_existing_image_marker(PROJECT_ROOT / "outputs" / "figures" / "linear_regression_predicted_vs_actual.png", "Linear Regression predicted vs actual plot.")}

{_existing_image_marker(PROJECT_ROOT / "outputs" / "figures" / "knn_regressor_predicted_vs_actual.png", "KNN Regressor predicted vs actual plot.")}

{_existing_image_marker(PROJECT_ROOT / "outputs" / "figures" / "linear_regression_residuals.png", "Linear Regression residuals.")}

{_existing_image_marker(PROJECT_ROOT / "outputs" / "figures" / "knn_regressor_residuals.png", "KNN Regressor residuals.")}

{_existing_image_marker(PROJECT_ROOT / "outputs" / "figures" / "regression_model_comparison.png", "Regression model comparison.")}
...
The image dataset used in the current run is `{image_meta.get("dataset_name", "Not available")}`. It is loaded from class folders inside `datasets/images/`.

Dataset details from the saved results:

- Image size used by the pipeline: {_fmt(image_meta.get("image_size", "Not available"))}
- Number of classes: {_fmt(image_meta.get("number_of_classes", "Not available"))}
- Class labels: {", ".join(image_classes) if image_classes else "Not available"}
- Total images: {_fmt(image_meta.get("split_counts", {}).get("total"))}
- Training images: {_fmt(image_meta.get("split_counts", {}).get("train"))}
- Validation images: {_fmt(image_meta.get("split_counts", {}).get("validation"))}
- Test images: {_fmt(image_meta.get("split_counts", {}).get("test"))}
- Extracted feature count: {_fmt(image_feature_count)}

Class counts:

{chr(10).join(f"- {label}: {_fmt(count)} images" for label, count in image_counts.items()) if image_counts else "- Not available"}
...
The feature extractor creates {_fmt(image_feature_count)} features per image. Each image is resized to `{image_meta.get("image_size", "Not available")}` and converted into numerical descriptors.
...
- Training feature matrix: {_fmt(image_meta.get("feature_dimensions", {}).get("train"))}
- Validation feature matrix: {_fmt(image_meta.get("feature_dimensions", {}).get("validation"))}
- Test feature matrix: {_fmt(image_meta.get("feature_dimensions", {}).get("test"))}
...
PCA is used selectively where it improves validation performance. In the saved Logistic Regression model, selected PCA components were `{logistic.get("hyperparameters", {}).get("selected_pca_n_components", "Not available")}`. PCA helps reduce noise and redundancy in the feature set while keeping the project classical and explainable.
...
- `C`: {logistic.get("hyperparameters", {}).get("selected_C", "Not available")}
- `max_iter`: {logistic.get("hyperparameters", {}).get("selected_max_iter", "Not available")}
- `class_weight`: {logistic.get("hyperparameters", {}).get("selected_class_weight", "Not available")}
- `selected_pca_n_components`: {logistic.get("hyperparameters", {}).get("selected_pca_n_components", "Not available")}
- `augmentation_used`: {logistic.get("hyperparameters", {}).get("augmentation_used", "Not available")}
...
In this project, `n_clusters` is set equal to the number of selected classes, which is {_fmt(image_meta.get("number_of_classes", "Not available"))}. The model creates that many clusters in feature space.
...
- `n_clusters`: {kmeans.get("hyperparameters", {}).get("n_clusters", "Not available")}
- `init`: {kmeans.get("hyperparameters", {}).get("init", "Not available")}
- `n_init`: {kmeans.get("hyperparameters", {}).get("selected_n_init", "Not available")}
- `max_iter`: {kmeans.get("hyperparameters", {}).get("max_iter", "Not available")}
- `selected_pca_n_components`: {kmeans.get("hyperparameters", {}).get("selected_pca_n_components", "Not available")}

Cluster-to-label mapping:

{chr(10).join(f"- Cluster {cluster}: {label}" for cluster, label in kmeans.get("cluster_to_label_mapping", {}).items()) if kmeans.get("cluster_to_label_mapping") else "- Not available"}
...
| Model | Test Accuracy |
|---|---:|
| Logistic Regression | {_percent(logistic_test.get("accuracy"))} |
| KMeans | {_percent(kmeans_test.get("accuracy"))} |

Logistic Regression performs better in the saved run. Its test accuracy is {_percent(logistic_test.get("accuracy"))}, while KMeans achieves {_percent(kmeans_test.get("accuracy"))}. This is expected because Logistic Regression is supervised and learns from true labels, while KMeans is unsupervised and only later mapped to labels.

{_existing_image_marker(PROJECT_ROOT / "outputs" / "figures" / "image_model_comparison.png", "Image model comparison.")}

{_existing_image_marker(PROJECT_ROOT / "outputs" / "confusion_matrices" / "logistic_regression_confusion_matrix.png", "Logistic Regression confusion matrix.")}

{_existing_image_marker(PROJECT_ROOT / "outputs" / "confusion_matrices" / "kmeans_confusion_matrix.png", "KMeans confusion matrix after majority-vote label mapping.")}
...
def _add_table(story: list[Any], header: list[str], rows: list[list[str]], styles: dict[str, ParagraphStyle]) -> None:
    data = [[Paragraph(_inline_markup(cell), styles["small"]) for cell in header]]
    for row in rows:
        data.append([Paragraph(_inline_markup(cell), styles["small"]) for cell in row])
...
def _markdown_to_story(markdown_text: str) -> list[Any]:
    styles = _styles()
    story: list[Any] = []
    ...
    def flush_bullets() -> None:
        ...
        if not bullet_buffer:
            return
        items = [
            ListItem(Paragraph(_inline_markup(item), styles["body"]), bulletColor=colors.HexColor("#2563eb"))
            for item in bullet_buffer
        ]
        story.append(ListFlowable(items, bulletType="bullet", leftIndent=18, bulletFontSize=6))
    ...
    def flush_code() -> None:
        ...
        if code_buffer:
            story.append(Preformatted("\n".join(code_buffer), styles["code"], maxLineLength=92))
            code_buffer = []
    ...
    def flush_table() -> None:
        ...
        for line in table_buffer:
            ...
            if all(set(part.replace(":", "").replace("-", "").strip()) == set() for part in parts):
                continue
            rows.append(parts)
        if rows:
            width = max(len(row) for row in rows)
            rows = [row + [""] * (width - len(row)) for row in rows]
            _add_table(story, rows[0], rows[1:], styles)
        table_buffer = []
    ...
    for raw_line in markdown_text.splitlines():
        ...
        if image_match:
            flush_bullets()
            rel_path, caption = image_match.groups()
            ...
            if image_path.exists():
                try:
                    ...
                    story.append(Paragraph(f"Figure could not be embedded: {_inline_markup(caption)}", styles["small"]))
            continue
        ...
        if line.startswith("> "):
            flush_bullets()
            story.append(Paragraph(_inline_markup(line[2:].strip()), styles["small"]))
            continue
...
def _page_decorator(canvas, doc) -> None:
    ...
    canvas.drawString(0.55 * inch, height - 0.22 * inch, "ML Project Discussion Preparation Guide")
...
def build_discussion_guide() -> tuple[Path, Path]:
    ...
    markdown_path = DOCS_DIR / "Project_Discussion_Preparation_Guide.md"
    pdf_path = DOCS_DIR / "Project_Discussion_Preparation_Guide.pdf"
    ...
    doc = SimpleDocTemplate(
        ...
        title="Project Discussion Preparation Guide",
